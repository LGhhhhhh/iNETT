...
w = np.loadtxt('E:/GH/AO/lungweight.txt')
...
for i in range(1000, 2000):
    ...
    train_set_data[i, ..., 0] = b_noisy #训练集投影数据（加噪声）
    print(i)
...
for i in range(1000, 2000):
    ...
    """1000个伪影的训练集"""
for i in range(1000):
    train_set[i, ..., 0] = train_set0[i, ..., 0]
    train_set_data[i] = train_set_data0[i]
    train_true_image = train_true_image0[i, ..., 0]
    label_set[i, ..., 0] = label_set0[i, ..., 0]
...
for i in range(200, 400):
    ...
    p = np.dot(w, np.reshape(f, [256 * 256, 1]))
    b_noisy = wgn_fun(p, noise_num)
    test_set_data[i, ..., 0] = b_noisy #训练集投影数据（加噪声）
    print(i)


    """200个无伪影的训练集"""
for i in range(200, 400):
    test_set[i, ..., 0] = test_true_image[i, ..., 0]
    print(i)
    """200个伪影的训练集"""
for i in range(200):
    test_set[i, ..., 0] = test_set0[i, ..., 0]
    test_set_data[i] = test_set_data0[i]
    test_true_image = test_true_image0[i, ..., 0]
    test_label_set[i, ..., 0] = test_label_set0[i, ..., 0]
...
for i in range(200):
    ...
    p = np.dot(w, np.reshape(f, [256 * 256, 1]))
    b_noisy = wgn_fun(p, noise_num)
    predict_set_data[i, ..., 0] = b_noisy #训练集投影数据（加噪声）

"""200个有伪影的测试集"""
for i in range(200):
    p = predict_set_data[i, ..., 0]
    rec, F = art_fun(w, p, k, r)
    rec = np.reshape(rec, [256, 256])
    predict_label_set[i, ..., 0] = predict_true_image[i, ..., 0] - rec  # 有伪影的label为伪影 （原始图像减去ART算法后的图像）
    predict_set[i, ..., 0] = rec
    print(i)

np.save(path1 + 'predict_true_image.npy', predict_true_image)
np.save(path1 + 'predict_set_data.npy', predict_set_data)
np.save(path1 + 'predict_set.npy', predict_set)
